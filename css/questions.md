...
- [JavaScript](#javascript)
  ...
  - [Q: 替换 URL 参数的同时不刷新网页，让用户第二次访问与第一次不同](#q-替换-url-参数的同时不刷新网页让用户第二次访问与第一次不同)
  - [Q: more...](#q-more)
...
- 多行文本超长打点
  ...
  多行文本超长打点的兼容性在 IE11 以上，有需要考虑兼容性的情况则可以参考 [Clamp.js](https://github.com/josephschmitt/Clamp.js)（一个封装好的多行溢出打点库）
...
- 情况：
  ...
  问题 1：在 javascript 中访问`elem.style.<attr>`，得到的属性返回空；

  问题 2：在 javascript 中使用`elem.style.<attr> = <val>`为样式规则赋值，样式没有应用上。
...
- 情况：
  ...
  最简单的替换 URL 思路是 `location.href = newUrl` 或 `location.replace(newUrl)`，但当我们设置该值时浏览器会自动刷新页面，这对用户的体验是不好的。

- 解决方法：

  借助 History 的 API [History.replaceState()](https://developer.mozilla.org/zh-CN/docs/Web/API/History/replaceState) 可以实现该效果：
